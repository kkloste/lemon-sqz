function [topk_stats] = seed_aug_experiment( A, C, varargin )
% [topk_stats] = seed_aug_experiment( A, C, varargin )
%
% INPUTS
%     A         - adjacency matrix
%     C         - community matrix
...
%                         or an integer k>=1 to use a k-walk; default = 2
%                         Input 0 to skip subgraph extraction.
...
n = size(A,1);
rmethod = p.Results.rank_method;
smethod = p.Results.subg_method;
...
for which_comm=1:NUM_COMMS,
    ...
    for which_seed=1:comm_size,  % COMPUTE ON EACH SEED
        ...
        % CALL ALGORITHM HERE
        new_seedset = seed_augment(A, seed, 'rank_method', rmethod, 'subg_method', smethod);
